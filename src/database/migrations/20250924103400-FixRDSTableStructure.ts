...
export class FixRDSTableStructure20250924103400 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const hasYearColumn = studentCountersColumns.some(
      (col: any) => col.column_name === 'year',
    );
    const hasYearMonthColumn = studentCountersColumns.some(
      (col: any) => col.column_name === 'yearMonth',
    );
    ...
    const hasIsPrimaryColumn = parentStudentsColumns.some(
      (col: any) => col.column_name === 'isPrimary',
    );
    ...
    const hasFeedbackModifiedByName = schedulesColumns.some(
      (col: any) => col.column_name === 'feedbackModifiedByName',
    );
    const hasFeedbackModifiedAt = schedulesColumns.some(
      (col: any) => col.column_name === 'feedbackModifiedAt',
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    if (parentStudentsColumns.length > 0) {
      await queryRunner.query(
        `ALTER TABLE "parent_students" DROP COLUMN "isPrimary"`,
      );
    }
    ...
    for (const column of schedulesColumns) {
      await queryRunner.query(
        `ALTER TABLE "schedules" DROP COLUMN "${column.column_name}"`,
      );
    }
  }
}
