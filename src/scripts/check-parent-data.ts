...
async function bootstrap() {
  ...
  for (const parent of parents) {
    if (!parent.email || parent.email.trim() === '') {
      emptyEmailCount++;
      console.log(
        `Parent ${parent.id} (${parent.name}) has missing/empty email.`,
      );
    }
    if (!parent.contactNo || parent.contactNo.trim() === '') {
      emptyPhoneCount++;
      console.log(
        `Parent ${parent.id} (${parent.name}) has missing/empty phone.`,
      );
    }
  }
  ...
  if (emptyPhoneCount > 0) {
    ...
    console.log(
      'Writing list of parents with missing phones to missing-phones.json...',
    );
    const missingPhones = parents.filter(
      (p) => !p.contactNo || p.contactNo.trim() === '',
    );
    fs.writeFileSync(
      'missing-phones.json',
      JSON.stringify(
