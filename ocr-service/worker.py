...
Reads image paths from stdin (one per line) and processes them.
Outputs JSON results to stdout (one per line).
Stays alive until EOF on stdin.
"""
...
# Redirect stdout to stderr to capture library noise during initialization
original_stdout = sys.stdout
sys.stderr.write("[Worker] Initializing PaddleOCR...\n")
sys.stderr.flush()
...
# PaddleOCR Singleton - Initialize once at startup (this is the expensive operation)
# This ensures only ONE instance exists throughout the worker's lifetime
_ocr_instance = None
...
def get_ocr():
    ...
    if _ocr_instance is None:
        sys.stderr.write("[Worker] Loading PaddleOCR models...\n")
        sys.stderr.flush()
        _ocr_instance = PaddleOCR(use_angle_cls=False, lang='en')
        sys.stderr.write("[Worker] Ready! Waiting for image paths...\n")
        sys.stderr.flush()
    return _ocr_instance

# Initialize OCR singleton at startup
ocr = get_ocr()

def process_image(image_path: str, exhaustive_mode: bool = False):
    """Process a single image and return formatted results"""
...
if __name__ == "__main__":
    ...
    for line in sys.stdin:
        ...
        # Parse input: "image_path" or "image_path --exhaustive"
        parts = line.split()
        image_path = parts[0]
        exhaustive_mode = "--exhaustive" in parts
        
        try:
            result = process_image(image_path, exhaustive_mode)
            # Output JSON result (one per line)
